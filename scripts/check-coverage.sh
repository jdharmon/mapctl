...
CACHE_PATH="${HOME}/Library/Caches/mapctl/swiftpm"
COVERAGE_BUILD_PATH="${ROOT_DIR}/.build/coverage"
mkdir -p "${CACHE_PATH}"

MIN_COVERAGE="${COVERAGE_MIN:-90}"
INCLUDE_REGEX="${COVERAGE_INCLUDE_REGEX:-/Sources/MapCore/}"
EXCLUDE_REGEX="${COVERAGE_EXCLUDE_REGEX:-/Sources/MapCore/(MapService|MapServiceDirections|MapKitConversions)\\.swift}"

echo "==> swift test --enable-code-coverage (isolated build dir)"
"${ROOT_DIR}/scripts/swift-test.sh" --enable-code-coverage --build-path "${COVERAGE_BUILD_PATH}" --cache-path "${CACHE_PATH}" >/dev/null

REPORT_JSON="$(
  find "${COVERAGE_BUILD_PATH}" -type f -path "*/codecov/mapctl.json" -print0 2>/dev/null \
    | xargs -0 ls -t 2>/dev/null \
    | head -n 1
)"

if [ -z "${REPORT_JSON}" ] || [ ! -f "${REPORT_JSON}" ]; then
  echo "ERROR: Coverage report not found (expected .build/**/codecov/mapctl.json)." >&2
  exit 1
fi
